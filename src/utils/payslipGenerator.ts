...
export function generatePayslipData(
  ...
  if (salaryRecord) {
    ...
    if (salaryRecord.careHours && salaryRecord.careRateAtTime) {
      regularPay = Math.round(salaryRecord.careHours * salaryRecord.careRateAtTime);
    }
    if (salaryRecord.officeHours && salaryRecord.officeRateAtTime) {
      officeWorkPay = Math.round(salaryRecord.officeHours * salaryRecord.officeRateAtTime);
    }
    ...
    // シフトデータから計算
    regularPay = Math.round(regularWorkHours * totalHourlyRate);
    accompanyingPay = Math.round(accompanyingHours * totalHourlyRate * 0.7); // 同行は70%
    nightPay = Math.round(nightWorkHours * totalHourlyRate * 1.25); // 深夜は125%
    officeWorkPay = Math.round(officeWorkHours * totalHourlyRate);
    salesWorkPay = Math.round(salesWorkHours * totalHourlyRate);
  }
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (dailyAttendanceMap[displayDate]) {
      dailyAttendance.push({
        ...dailyAttendanceMap[displayDate],
        regularWork: Math.round(dailyAttendanceMap[displayDate].regularWork * 10) / 10,
        regularNight: Math.round(dailyAttendanceMap[displayDate].regularNight * 10) / 10,
        accompanyingWork: Math.round(dailyAttendanceMap[displayDate].accompanyingWork * 10) / 10,
        accompanyingNight: Math.round(dailyAttendanceMap[displayDate].accompanyingNight * 10) / 10,
        officeWork: Math.round(dailyAttendanceMap[displayDate].officeWork * 10) / 10,
        salesWork: Math.round(dailyAttendanceMap[displayDate].salesWork * 10) / 10,
        totalHours: Math.round(dailyAttendanceMap[displayDate].totalHours * 10) / 10,
      });
    } else {
      dailyAttendance.push({
        date: displayDate,
  ...
  return {
    ...
    attendance: {
      ...
      lateDays: 0,
      totalWorkDays,
      regularWorkHours: Math.round(regularWorkHours * 10) / 10,
      accompanyingHours: Math.round(accompanyingHours * 10) / 10,
      nightWorkHours: Math.round(nightWorkHours * 10) / 10,
      officeWorkHours: Math.round(officeWorkHours * 10) / 10,
      salesWorkHours: Math.round(salesWorkHours * 10) / 10,
      totalWorkHours: Math.round(totalWorkHours * 10) / 10,
      nightAccompanyingHours: Math.round(nightAccompanyingHours * 10) / 10,
      officeHoursDetail: Math.round(officeWorkHours * 10) / 10,
      salesHoursDetail: Math.round(salesWorkHours * 10) / 10,
    },
