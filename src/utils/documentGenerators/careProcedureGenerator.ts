    ...
    for (const proc of manual.procedures) {
      ...
      if (houseStepCount > proc.steps.length / 2) {
        ...
        if (planBlock && planBlock.steps.length > 0) {
          ...
          // ★ current journals 実態に合わせて固定: 訪問時確認・服薬確認・排泄介助・食事見守り
          const bodySteps = [
            ...
            { item: '排泄介助', detail: 'トイレ誘導、排泄後の清拭、衣服の整え等を必要に応じて行う', note: '清潔保持と転倒予防に配慮する' },
            { item: '食事見守り', detail: '食事量・水分摂取量を確認し、見守りと声かけを行う。必要時のみ配膳補助を行う', note: 'むせ込みや摂取状況に注意し、食事介助と誤読される表現は使わない' },
          ];
