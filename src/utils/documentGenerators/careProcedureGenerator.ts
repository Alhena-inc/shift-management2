    ...
    for (const proc of manual.procedures) {
      ...
      if (houseStepCount > proc.steps.length / 2) {
        ...
        if (planBlock && planBlock.steps.length > 0) {
          ...
          const bodySteps = [
            { item: '体調確認', detail: '顔色・表情・体調を確認し、体調・気分・食欲等について聴取する。必要時に体温を測定し、異常時はサービス提供責任者に報告する', note: '自覚症状の訴えに注意し、変化があれば記録。体温を測定した場合は記録し平熱との差があれば報告する' },
  ...
  for (const proc of manual.procedures) {
    ...
    if (!hasHealthCheck) {
      ...
      const healthCheckStep: ProcedureStep = {
        ...
        item: '体調確認',
        detail: '顔色・表情・体調を確認し、体調・気分・食欲等について聴取する。必要時に体温を測定し、異常時はサービス提供責任者に報告する。',
        note: '自覚症状の訴えに注意。体温を測定した場合は記録し、平熱との差や体調変化があれば報告する。',
      };
      // 挨拶ステップの後に挿入
