    ...
    for (const proc of manual.procedures) {
      ...
      if (houseStepCount > proc.steps.length / 2) {
        ...
        if (planBlock && planBlock.steps.length > 0) {
          ...
          // ★ current journals 実態: 更衣51件・整容46件・安全確認42件・傾聴33件、食事0件
          const bodySteps = [
            ...
            { item: '洗面・整容支援', detail: '洗面・歯磨き等の整容支援を必要に応じて行う', note: '本人動作を活かしつつ清潔保持を支援する' },
            { item: '更衣支援', detail: '就寝前の更衣介助を必要に応じて行う', note: '肩の可動域や手足の震えに配慮する' },
            { item: '相談・傾聴', detail: '利用者の不安や訴えを傾聴し、相談援助を行う', note: '表情の変化に注意し丁寧に対応する' },
