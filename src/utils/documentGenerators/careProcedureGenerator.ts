    ...
    for (const proc of manual.procedures) {
      ...
      if (houseStepCount > proc.steps.length / 2) {
        ...
        if (planBlock && planBlock.steps.length > 0) {
          ...
          // ★ current journals 実態に合わせて固定: 訪問時確認・服薬確認・食事見守り・水分補給・食後整容支援・必要時移動見守り
          const bodySteps = [
            { item: '訪問時確認', detail: '訪問時に表情・体調・食欲の有無を確認する。体調変化や訴えがあれば記録し必要時報告する', note: '著変や食欲低下の有無に注意する' },
            { item: '服薬確認', detail: '処方薬を確認し、本日分の服薬状況を確認する。必要時は声かけを行い確実な服薬を見届ける', note: '漢方薬への抵抗感に配慮し無理強いせず確認する' },
            { item: '食事見守り', detail: '食事の配膳を整え、摂取状況を確認しながら見守りと声かけを行う', note: 'むせ込みや摂取量に注意し、食事介助と誤読される表現は使わない' },
            { item: '水分補給促し', detail: '食後に水分摂取を促し、摂取量を確認する', note: '脱水予防のため声かけを行う' },
            { item: '食後整容支援', detail: '歯磨き・洗顔等の整容支援を必要に応じて行う', note: '本人動作を活かしつつ清潔保持を支援する' },
            { item: '必要時移動見守り', detail: '移動時のふらつきや転倒に注意し、必要時に付き添いを行う', note: '足の震えやつまずきに配慮する' },
          ];
