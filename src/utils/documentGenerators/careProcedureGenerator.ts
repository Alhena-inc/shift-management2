    ...
    for (const proc of manual.procedures) {
      ...
      if (houseStepCount > proc.steps.length / 2) {
        ...
        if (planBlock && planBlock.steps.length > 0) {
          ...
          // ★入浴介助が日誌実態の主軸である場合は入浴ベースのステップを使用
          // ★「バイタルチェック」と「体調確認」を統合 → 「体調確認」に一本化
          // 日誌側は実測値がある場合のみバイタル確認ON、なければ「体調確認を実施」とする運用に整合。
          // 手順書に「毎回測定」と書くと、測定していない日の日誌と矛盾するため。
          // ★入浴の有無を計画書サービスブロックから判定
          const planBlockForBodyCheck = ctx.carePlanServiceBlocks?.find(b => b.service_type.includes('身体'));
          const planBodyText = planBlockForBodyCheck ? planBlockForBodyCheck.steps.map(s => `${s.item} ${s.content}`).join(' ') : '';
          const hasBathInPlan = /入浴|シャワー|清拭|洗体|洗髪/.test(planBodyText);
          const bodySteps = hasBathInPlan ? [
            { item: '服薬確認', detail: '処方薬の服薬状況を確認し、飲み忘れや残薬がないか確認する。必要に応じて声かけを行う', note: '服薬している漢方薬に対して嫌悪感があるため、毎回必ず服薬状況を確認し、飲み忘れや拒否がないか注意する' },
            { item: '入浴介助', detail: 'シャワー浴または入浴の準備を行い、浴室内での安全に配慮しながら見守りまたは必要な介助を行う', note: '転倒やふらつきに注意し、必要時は移動介助を行う' },
            { item: '更衣・整容介助', detail: '入浴後または就寝前の更衣介助、洗面・歯磨き等の整容介助を必要に応じて行う', note: '肩の可動域や手足の震えに配慮し、無理のない範囲で本人動作を活かす' },
            { item: '就寝前安全確認', detail: '室内環境、戸締り、火元等を確認し、安心して就寝できる状態を整える', note: '転倒防止のため動線や寝室環境に配慮する' },
          ] : [
            { item: '服薬確認', detail: '処方薬の服薬状況を確認し、飲み忘れや残薬がないか確認する。必要に応じて声かけを行う', note: '服薬時間・量を確認し、飲み忘れがあれば速やかに対応する' },
            { item: '体調確認', detail: '体調を確認し、必要時に体温を測定する。異常時はサービス提供責任者に報告する', note: '体温を測定した場合は記録し、平熱との差や体調変化があれば報告する' },
