...
- 【シフト・実績情報】の「サービス種別×時間帯」パターンごとに1つのProcedureBlockを生成
- 同じサービス種別・同じ時間帯パターン（開始時刻・終了時刻が同じ）は1つにまとめる
- 曜日が異なっても種別・時間帯が同じなら1つのブロックにする
- ★重要：身体介護と家事援助は別ブロックとすること（同じ時間帯でもサービス種別が異なれば別ブロック）
...
- visit_label: 「月・水・金 11:00〜12:00 身体介護」形式の訪問ラベル
- steps: 10〜15件の具体的な手順ステップ
- ★重要：身体介護ブロックには身体系ケアの手順のみ、家事援助ブロックには家事系ケアの手順のみ記載。混ぜないこと。重度訪問介護は混在OK
...
【出力ルール】
1. proceduresの数は「サービス種別×時間帯」パターン数と一致させる
2. 各ブロックのservice_typeは計画書と一致させること
3. ★身体介護ブロックには身体系ケアのみ、家事援助ブロックには家事系ケアのみ記載
...
function buildBillingSummary(records: BillingRecord[]): string {
  ...
  lines.push(`→ 身体介護ブロックには身体系ケアの手順のみ、家事援助ブロックには家事系ケアの手順のみ記載すること`);
