...
- 実績記録なしでのモニタリング報告書生成は本来認められない（システム側でブロック済み）
...
## 計画変更の要否判断
以下の場合は plan_revision_needed を「あり」に設定すること:
- 利用者の心身状態に大きな変化がある（状態悪化・改善ともに）
- 目標の達成状況が著しく乖離している（ほぼすべて未達成等）
- 生活環境に大きな変化がある（転居、家族構成変化、入退院等）
- サービス内容と利用者ニーズにミスマッチがある

## 禁止事項
- 実績記録なしでの根拠のないモニタリング報告書生成
- 計画書のサービス内容に対するモニタリング項目の欠落
- テンプレート的な評価文（個別性のある記述をすること）
