...
※ service_reasonの時間枠ごとの種別記載は、実績データのサービスコード（service_types）と一致させること
...
export async function generate(ctx: GeneratorContext): Promise<{ planRevisionNeeded: string; monitoringCycleMonths: number; goalContinuation: boolean }> {
  ...
  if (ctx.monitoringType === 'short_term') {
    monitoringTriggerNote = '\n\n【モニタリング実施理由】\nこのモニタリングは短期目標の期間満了に伴い実施するものです。goal_evaluation欄に「短期目標の期間満了に伴うモニタリング」である旨を冒頭に記載してください。';
  } else if (ctx.monitoringType === 'long_term') {
    monitoringTriggerNote = '\n\n【モニタリング実施理由】\nこのモニタリングは長期目標の期間満了に伴い実施するものです。goal_evaluation欄に「長期目標の期間満了に伴うモニタリング」である旨を冒頭に記載してください。';
  }
  ...
  // ★重要: 同一時間枠が帳票ごとに異なるサービスとして読まれる問題を防止する。
  // 実績データからtime_slot別のservice_typeを構築し、D12の記載と一致させる。
  if (serviceTypes.length > 0 && result.service_reason) {
    ...
    for (const [time, actualType] of timeSlotTypes) {
      // 「18:30」「19:30」等の時間帯が記載されている場合に種別を修正
      const timePattern = new RegExp(`${time.replace(':', '[：:]?')}[^。]*?(身体介護|家事援助|重度訪問|通院)`);
      const match = reason.match(timePattern);
      if (match && match[1] !== actualType && !actualType.includes('重度')) {
        console.log(`[Monitoring] D12時間枠修正: ${time}枠 「${match[1]}」→「${actualType}」（実績ベース）`);
        reason = reason.replace(match[0], match[0].replace(match[1], actualType));
      }
    }
