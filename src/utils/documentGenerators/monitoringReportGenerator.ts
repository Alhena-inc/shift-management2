...
【達成の場合】
「短期目標『（目標文言をそのまま引用）』について、（具体的な状況・根拠）により、
目標を達成したと判断する。」

【継続の場合】
「短期目標『（目標文言をそのまま引用）』について、（具体的な状況）は安定しているが、
定着のため引き続き支援が必要と判断し、目標を継続する。」
★重要：「目標継続」と判断した場合、次の計画書の目標文言は前の計画書と完全に同一にすること。
目標の内容や表現を少しでも変える場合は「目標継続」ではなく「目標変更」を選ぶこと。

【変更の場合】
「短期目標『（目標文言をそのまま引用）』について、（理由）により目標を達成したため、
次の段階として新たな短期目標を設定する。」
...

{
  ...
  "goal_evaluation": "短期目標『○○』について、○○の状況により目標を継続する。",
