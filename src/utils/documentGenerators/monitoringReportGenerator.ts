...

{
  ...
  "service_change": 1,
  "service_change_reason": "確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。清掃を中心とした家事援助と、身体介護時の体調確認・服薬確認・食事見守り支援の継続により、現状の生活が安定して維持できている。",
  ...
  "procedure_check": "手順書の内容を見直し、体調確認を基本とし必要時のみ体温を測定する運用に統一したため、現行サービスに適切に対応できる内容となった。掃除箇所はその日の状況に応じて臨機応変に対応する方針を維持する。"
}
...
export async function generate(ctx: GeneratorContext): Promise<{
  ...
  const result: MonitoringResult = {
    ...
    procedure_check: (rawJson.procedure_check as string) || '手順書の内容を見直し、体調確認を基本とし必要時のみ体温を測定する運用に統一したため、現行サービスに適切に対応できる内容となった。掃除箇所はその日の状況に応じて臨機応変に対応する方針を維持する。',
  };
  ...
  const reasonFields: Array<{ key: keyof MonitoringResult; fallback: string }> = [
    ...
    { key: 'service_change_reason', fallback: '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。清掃を中心とした家事援助と、身体介護時の体調確認・服薬確認・食事見守り支援の継続により、現状の生活が安定して維持できている。' },
  ];
