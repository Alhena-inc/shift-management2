...

{
  ...
  "service_change": 1,
  "service_change_reason": "確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、身体介護時の服薬確認・安全確認・相談援助が適切に実施され、在宅生活の安定が図られている。",
  ...
  "procedure_check": "手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、抗酒剤の服薬状況確認、室内安全確認、相談・傾聴、就寝準備および退室前安全確認の手順について、現在の手順で支援が適切に実施されている。"
}
...
export async function generate(ctx: GeneratorContext): Promise<{
  ...
  const result: MonitoringResult = {
    ...
    procedure_check: (rawJson.procedure_check as string) || '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、抗酒剤の服薬状況確認、室内安全確認、相談・傾聴、就寝準備および退室前安全確認の手順について、現在の手順で支援が適切に実施されている。',
  };
  ...
  const TYPO_FIXES: Array<[RegExp, string]> = [
    ...
    [/健康チェック/g, '体調確認'],
    // ★ current journals 実態統一: 食事見守り/排泄/入浴/移動見守りは使わない
    // source-of-truth: 服薬確認68件・安全確認42件・傾聴33件、更衣0件・整容0件・食事0件
    [/食事見守り/g, '安全確認'],
    [/排泄介助/g, '安全確認'],
    [/入浴支援/g, '相談援助'],
    [/移動見守り/g, '安全確認'],
    [/必要時の更衣・整容支援/g, '安全確認・相談援助'],
    [/洗面・整容支援/g, '安全確認'],
    [/更衣支援/g, '相談援助'],
  ...
  const reasonFields: Array<{ key: keyof MonitoringResult; fallback: string }> = [
    ...
    { key: 'service_change_reason', fallback: '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、身体介護時の服薬確認・安全確認・相談援助が適切に実施され、在宅生活の安定が図られている。' },
  ];
  ...
  if (result.procedure_check) {
    ...
    if (!result.procedure_check.trimEnd().endsWith('。')) {
      ...
      if (INCOMPLETE_END.test(body) || body.length < 30) {
        ...
        result.procedure_check = '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、抗酒剤の服薬状況確認、室内安全確認、相談・傾聴、就寝準備および退室前安全確認の手順について、現在の手順で支援が適切に実施されている。';
      } else {
        // 末尾に「。」を追加して完結させる
