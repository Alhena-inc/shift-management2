...

{
  ...
  "service_change": 1,
  "service_change_reason": "確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、身体介護時の服薬確認・食事見守り・必要時の移動見守りおよび整容支援が適切に実施され、在宅生活の安定が図られている。",
  ...
  "procedure_check": "手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、食事見守り、水分補給の促し、必要時の整容支援および移動見守りの手順について、現在の手順で支援が適切に実施されている。"
}
...
export async function generate(ctx: GeneratorContext): Promise<{
  ...
  const result: MonitoringResult = {
    ...
    procedure_check: (rawJson.procedure_check as string) || '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、食事見守り、水分補給の促し、必要時の整容支援および移動見守りの手順について、現在の手順で支援が適切に実施されている。',
  };
  ...
  const TYPO_FIXES: Array<[RegExp, string]> = [
    ...
    [/健康チェック/g, '体調確認'],
    // ★ current journals 実態統一: 排泄介助は使わない
    [/排泄介助・食事見守り/g, '食事見守り'],
    [/排泄介助および食事見守り/g, '食事見守り'],
    [/入浴支援/g, '食事見守り'],
    [/入浴時の見守り・介助支援/g, '食事見守り・必要時移動見守り'],
  ...
  const reasonFields: Array<{ key: keyof MonitoringResult; fallback: string }> = [
    ...
    { key: 'service_change_reason', fallback: '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、身体介護時の服薬確認・食事見守り・必要時の移動見守りおよび整容支援が適切に実施され、在宅生活の安定が図られている。' },
  ];
