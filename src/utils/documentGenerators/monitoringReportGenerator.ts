...
※ 実施状況の記載は計画予定表（グリッド）に入力されている全曜日・全サービス種別を漏れなく記載すること
※ 家事援助と身体介護が別時間帯で提供されている場合はそれぞれの時間帯・種別・内容を明記すること
※ 同一種別が複数時間帯にある場合は時間帯ごとに記載すること
※ service_reasonの時間枠ごとの種別記載は、下記【各時間枠のサービス種別】欄の多数決結果と一致させること
※ ★重要: 同一時間帯で曜日ごとに実績コードが異なる場合があるが、各時間枠の真値（多数決結果）を一貫して使うこと。曜日ごとに種別を変えてはいけない
※ 2,3の場合はその理由を60〜120文字で記載
※ 1の場合も「計画に基づき（具体的なサービス内容と曜日）が提供されていることを確認した」と記載
...
{
  "service_status": 1,
  "service_reason": "計画に基づき○○が提供されていることを確認した",
  "satisfaction": 1,
  ...
  "goal_evaluation": "短期目標『（【現在の目標達成状況】欄の短期目標文言をここにそのままコピー）』について、○○の状況により目標を継続する。長期目標『（同欄の長期目標文言をそのままコピー）』について、…",
  "procedure_check": "手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。"
}
...
function buildBillingSummary(records: BillingRecord[]): string {
  ...
  if (timeSlotCounts.size > 0) {
    ...
    lines.push('★重要: service_reason欄で各時間枠のサービス種別を記載する際は、上記の種別を使用すること。実績コードの曜日別バラつきではなく、多数決で決定した真値を使用する。');
  }
...
export async function generate(ctx: GeneratorContext): Promise<{ planRevisionNeeded: string; monitoringCycleMonths: number; goalContinuation: boolean }> {
  ...
  // 居宅介護計画書のサービスブロック情報をプロンプトに追加
  // ★最重要: D12(service_reason)の時間枠×種別は、この計画書情報を真値とすること
  ...
  if (ctx.carePlanServiceBlocks && ctx.carePlanServiceBlocks.length > 0) {
    const blockLines = ctx.carePlanServiceBlocks.map((block, i) => {
      const items = block.steps.slice(0, 3).map(s => s.item).join('・');
      return `  ブロック${i + 1}: ${block.service_type}（${block.visit_label}）→ ${items}`;
    });
    carePlanServiceNote = `\n\n【居宅介護計画書のサービス内容（D12の真値）】\n以下は直前の居宅介護計画書に記載されたサービス内容です。\nservice_reason欄で各時間枠のサービス種別を記載する際は、必ずこの計画書の種別に合わせてください。\n実績データのサービスコードが曜日によってブレている場合でも、計画書の種別を優先すること。\n${blockLines.join('\n')}`;
    console.log(`[Monitoring] 計画書サービスブロック情報をプロンプトに追加: ${ctx.carePlanServiceBlocks.length}ブロック`);
  }
