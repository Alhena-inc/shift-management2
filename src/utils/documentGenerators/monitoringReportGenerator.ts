...

{
  ...
  "service_change": 1,
  "service_change_reason": "確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、服薬状況の毎回確認、入浴時の見守り・介助支援の継続により、現状の生活が安定して維持できている。",
  ...
  "procedure_check": "手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、入浴介助・更衣介助・整容介助の手順について、現在の手順で支援が適切に実施されている。"
}
...
export async function generate(ctx: GeneratorContext): Promise<{
  ...
  const result: MonitoringResult = {
    ...
    procedure_check: (rawJson.procedure_check as string) || '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、入浴介助・更衣介助・整容介助の手順について、現在の手順で支援が適切に実施されている。',
  };
  ...
  const TYPO_FIXES: Array<[RegExp, string]> = [
    ...
    [/健康チェック/g, '体調確認'],
    // ★「食事見守り支援」→「入浴時の見守り・介助支援」に正規化（current operation: 入浴介助・見守り）
    [/食事見守り支援/g, '入浴時の見守り・介助支援'],
    // ★「入浴準備」（単体サマリー）→「入浴支援」に正規化
    [/身体介護（([^）]*)入浴準備([^）]*)）/g, '身体介護（$1入浴支援$2）'],
    // ★「清掃を中心とした家事援助」→「調理・掃除を中心とした家事援助」に正規化
    [/清掃を中心とした家事援助/g, '調理・掃除を中心とした家事援助'],
    // ★「入浴時の見守り支援」→「入浴時の見守り・介助支援」に正規化（介助も含む）
    [/入浴時の見守り支援/g, '入浴時の見守り・介助支援'],
  ];
  ...
  const reasonFields: Array<{ key: keyof MonitoringResult; fallback: string }> = [
    ...
    { key: 'service_change_reason', fallback: '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、服薬状況の毎回確認、入浴時の見守り・介助支援の継続により、現状の生活が安定して維持できている。' },
  ];
  ...
  if (result.procedure_check) {
    ...
    if (!result.procedure_check.trimEnd().endsWith('。')) {
      ...
      if (INCOMPLETE_END.test(body) || body.length < 30) {
        ...
        result.procedure_check = '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、入浴介助・更衣介助・整容介助の手順について、現在の手順で支援が適切に実施されている。';
      } else {
        // 末尾に「。」を追加して完結させる
