...

{
  ...
  "service_change": 1,
  "service_change_reason": "確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、身体介護時の服薬確認・更衣介助・整容支援・安全確認・相談援助が適切に実施され、在宅生活の安定が図られている。",
  ...
  "procedure_check": "手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、抗酒剤の服薬状況確認、更衣介助、必要時の整容支援、室内安全確認、相談・傾聴、就寝準備および退室前安全確認の手順について、現在の手順で支援が適切に実施されている。"
}
...
export async function generate(ctx: GeneratorContext): Promise<{
  ...
  const result: MonitoringResult = {
    ...
    procedure_check: (rawJson.procedure_check as string) || '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、抗酒剤の服薬状況確認、更衣介助、必要時の整容支援、室内安全確認、相談・傾聴、就寝準備および退室前安全確認の手順について、現在の手順で支援が適切に実施されている。',
  };
  ...
  const TYPO_FIXES: Array<[RegExp, string]> = [
    ...
    [/身体介護（体温測定・服薬確認・食事見守り・体調確認）/g, '身体介護（体調確認・服薬確認・更衣介助・整容支援・安全確認・相談援助）'],
    [/バイタルチェック/g, '体調確認'],
    // ★「健康チェック」→「体調確認」に正規化（current operation 統一）
    [/健康チェック/g, '体調確認'],
    // ★ current journals 実態統一: 食事見守り/排泄/入浴は日誌0件なので置換
    // source-of-truth: 服薬確認68件・更衣51件・整容46件・安全確認42件・傾聴33件、食事0件
    [/食事見守り/g, '整容支援'],
    [/排泄介助/g, '安全確認'],
    [/入浴支援/g, '整容支援'],
    [/移動見守り/g, '安全確認'],
    // ★ 薬剤名の混成表現を遮断（AIが「抗酒剤等の漢方薬」と生成する場合の対策）
    [/抗酒剤等の漢方薬/g, '抗酒剤'],
    [/漢方薬/g, '抗酒剤'],
  ...
  const reasonFields: Array<{ key: keyof MonitoringResult; fallback: string }> = [
    ...
    { key: 'service_change_reason', fallback: '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、身体介護時の服薬確認・更衣介助・整容支援・安全確認・相談援助が適切に実施され、在宅生活の安定が図られている。' },
  ];
  ...
  if (result.service_reason) {
    ...
    if (!result.service_reason || result.service_reason.length < 10) {
      ...
      if (hasBody && hasHouse) {
        result.service_reason = '現行計画に基づく家事援助及び身体介護は概ね提供できており、在宅生活は安定して継続されている。手の震えに配慮した支援により食生活も維持され、服薬確認・更衣介助を含む見守りにより心身の状態は概ね安定している。';
      } else if (hasBody) {
  ...
  // AIがアセスメントから無差別に引っ張る問題（訪問看護の服薬管理、漢方薬嫌悪、作業所活動等）への対策。
  {
    const CONTAMINATION_PATTERNS: Array<[RegExp, string]> = [
      ...
      [/抗酒剤[^、。]{0,30}(嫌[がい]?|嫌悪|苦手|飲み?たくない|拒否|抵抗)[^。]*。?/g, '抗酒剤への嫌悪感記述'],
  ...
  if (result.procedure_check) {
    ...
    if (!result.procedure_check.trimEnd().endsWith('。')) {
      ...
      if (INCOMPLETE_END.test(body) || body.length < 30) {
        ...
        result.procedure_check = '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、抗酒剤の服薬状況確認、更衣介助、必要時の整容支援、室内安全確認、相談・傾聴、就寝準備および退室前安全確認の手順について、現在の手順で支援が適切に実施されている。';
      } else {
        // 末尾に「。」を追加して完結させる
  ...
  // === J12 (condition_detail) 未完文ガード ===
  // AI生成文が途中で切れている場合（「通院と。」等）にフォールバック
  if (result.condition_detail) {
    const cd = result.condition_detail.trimEnd();
    const INCOMPLETE_CD = /[はがをにでもへとのやかし、,と]$/;
    // 「。」直前が助詞「と」で終わる未完文パターン（「クリニック通院と。」等）
    const BROKEN_SENTENCE = /[とにをがで]。$/;
    if (INCOMPLETE_CD.test(cd.replace(/。$/, '')) || BROKEN_SENTENCE.test(cd) || cd.length < 20) {
      console.warn(`[Monitoring] ⚠ J12(condition_detail)が未完文。フォールバックに差し替え: "${cd.substring(cd.length - 20)}"`);
      result.condition_detail = '身体状況・精神状態について確認し、前回モニタリング時と比較して著変なし。抗酒剤の継続服用により断酒生活が維持されており、定期的なクリニック通院も継続している。';
    } else if (!cd.endsWith('。')) {
      result.condition_detail = cd + '。';
    }
  }
