...
### ブロック数決定の基本ルール
サービス内容のブロック数は、【計画予定表（シフト・実績情報）】の時間帯パターンの数によって決まる。
...
3. 各ブロックは、対応する時間帯パターンで実施するサービス内容をまとめて記載する

### 「異なる時間帯パターン」の定義
- 開始時刻・終了時刻・提供内容の性質が同一であれば、曜日が異なっても「同じパターン」とみなす
- 開始時刻または終了時刻が異なる場合、または提供内容の性質が明らかに異なる場合は「別パターン」とみなす

### 具体例
#### 例1：2パターンの場合 → ブロックは2つ（service1とservice2のみ、service3・service4はnull）
計画予定表：土曜 11:00〜（日中の家事援助）、金・土・日 23:00〜（夜間の身体介護・見守り）
→ パターンA：日中帯（家事援助中心）→ service1
→ パターンB：夜間帯（身体介護・見守り中心）→ service2

#### 例2：1パターンの場合 → ブロックは1つ（service1のみ、service2〜4はnull）
計画予定表：月〜金 9:00〜12:00（身体介護＋家事援助）
→ パターンA：午前帯（身体介護＋家事援助）→ service1

#### 例3：3パターンの場合 → ブロックは3つ（service1〜3、service4はnull）
計画予定表：月・水・金 9:00〜12:00（身体介護）、火・木 14:00〜16:00（家事援助）、毎日 22:00〜翌6:00（見守り）
→ パターンA → service1、パターンB → service2、パターンC → service3

### やってはいけないこと
- 同じ時間帯パターンの内容を「身体介護」と「生活援助」で分割して別ブロックにする
- 同じ時間帯パターンなのに曜日ごとにブロックを分ける
- 計画予定表の時間帯パターン数と異なる数のブロックを生成する
...
■ 生成手順（必ずこの順番で思考すること）
═══════════════════════════════════════════════════
1. まず【シフト・実績情報】から時間帯パターンを抽出する
2. パターン数を確定する
3. 確定したパターン数と同数のサービス内容ブロック（service1〜）を生成する
4. 生成後、ブロック数が時間帯パターン数と一致しているか自己検証する
...
## サービス枠の分け方（最重要）
- サービス内容のブロック数は、計画予定表（シフト・実績情報）の「異なる時間帯パターン」の数と厳密に一致させる
- 開始時刻・終了時刻・提供内容の性質が同一であれば、曜日が異なっても「同じパターン」とみなす
- 同じ時間帯パターンの内容を「身体介護」と「家事援助」で分割して別ブロックにしない
- 同じ時間帯パターンなのに曜日ごとにブロックを分けない
- 重度訪問介護は身体・家事の混在を許可する
- まず時間帯パターンを列挙し、次にブロック数を確定してから生成すること
