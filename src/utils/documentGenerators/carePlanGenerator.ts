...
### 各ブロックの記載ルール
- 1回の訪問で実施するケアの流れ（手順）を時系列で記載する（身体介護→家事援助の順など）
...
■ 生成手順（必ずこの順番で思考すること）
═══════════════════════════════════════════════════
1. まず【シフト・実績情報】から「異なる時間帯」のパターンを抽出する（内容の種類ではなく、開始時刻・終了時刻のみで判定）
2. パターン数を確定する
3. アセスメントの援助項目を、該当する時間帯パターンごとに振り分ける
4. 確定したパターン数と同数のサービス内容ブロック（service1〜）を生成する（各ブロックには身体介護・家事援助が混在してよい）
5. 生成後、ブロック数が時間帯パターン数と一致しているか自己検証する
...
## サービス枠の分け方（最重要）
- ブロック数は計画予定表（シフト・実績情報）の「異なる時間帯」の数と厳密に一致させる
- 時間帯の判定は開始時刻・終了時刻のみで行う。援助内容の種類（身体介護/家事援助）でブロックを分けない
- アセスメントに身体介護と家事援助の両方の項目がある場合でも、同じ時間帯なら1ブロックにまとめる
