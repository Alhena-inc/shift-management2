...
### ブロック数決定の基本ルール（最重要）
サービス内容のブロック数は、【時間帯パターン判定結果】に記載されたパターン数と完全に一致させること。
パターンは「サービス種別 × 時間帯」の組み合わせで決まる。

1. 上記の【シフト・実績情報】末尾の【時間帯パターン判定結果】を確認する
2. パターン数 ＝ サービス内容ブロック（service1〜service4）の数（厳密に一致させること）
3. 各ブロックは、対応するパターンのサービス種別に合った内容のみを記載する
4. ★重要：身体介護ブロックには身体介護のケア内容のみ、家事援助ブロックには家事援助のケア内容のみを記載する。混ぜてはいけない。

### 「サービスパターン」の定義
- サービス種別（身体介護・家事援助・重度訪問介護等）と時間帯（開始〜終了）の組み合わせがパターン
- 同じ時間帯でもサービス種別が異なれば「別パターン」とみなす
- 同じ種別・同じ時間帯であれば曜日が異なっても「同じパターン」とみなす

### 具体例
#### 例1：身体介護と家事援助が別時間帯の場合 → ブロックは2つ
パターン判定結果：家事援助 19:00〜20:00、身体介護 20:00〜21:00
→ service1（service_type="家事援助"）：掃除・洗濯・調理等の家事系ケアのみ記載。全stepsのcategoryは「家事援助」
→ service2（service_type="身体介護"）：体調確認・排泄介助・更衣等の身体系ケアのみ記載。全stepsのcategoryは「身体介護」
→ service3・service4はnull

#### 例2：重度訪問介護で1パターンの場合 → ブロックは1つ
パターン判定結果：重度訪問 23:00〜翌7:00
→ service1（service_type="重度訪問介護"）：身体介護と家事援助が一体的に提供されるため混在OK
→ service2〜4はnull

#### 例3：3パターンの場合 → ブロックは3つ
パターン判定結果：身体介護 9:00〜10:00、家事援助 10:00〜12:00、身体介護 22:00〜翌6:00
→ service1（service_type="身体介護"）：午前の身体系ケア
→ service2（service_type="家事援助"）：午前の家事系ケア
→ service3（service_type="身体介護"）：夜間の身体系ケア
→ service4はnull

### やってはいけないこと（厳守）
- ★絶対禁止：身体介護ブロックに家事援助の内容（掃除・洗濯・調理等）を含めること
- ★絶対禁止：家事援助ブロックに身体介護の内容（排泄介助・入浴介助・移動介助等）を含めること
- ★絶対禁止：パターン判定結果と異なる数のブロックを生成すること
- ★絶対禁止：1回の長時間訪問の中の時間経過を理由にブロックを勝手に分割すること
- 同じサービス種別・同じ時間帯パターンなのに曜日ごとにブロックを分けること
- ※例外：重度訪問介護は身体介護と家事援助が一体的に提供されるため、1ブロック内に混在してよい

### 各ブロックの記載ルール
- 1回の訪問で実施するケアの流れ（手順）を時系列で記載する
- ★重要：各ステップの「content」の先頭に、そのケアを実施する具体的な時刻を記載すること
  - 例：パターンが12:00〜13:00の身体介護の場合
    - ステップ1: content = "12:00 入室し、体調確認・バイタルチェックを行う。..."
    - ステップ2: content = "12:10 排泄介助。トイレへの移動を見守り、..."
    - ステップ3: content = "12:30 入浴介助。浴室への移動を介助し、..."
    - ステップ4: content = "12:50 口腔ケア・整容を行い、..."
  - 例：パターンが10:00〜11:00の家事援助の場合
    - ステップ1: content = "10:00 居室の掃除機がけ・整理整頓を行う。..."
    - ステップ2: content = "10:15 洗濯物の取り込み・たたみ・収納を行う。..."
    - ステップ3: content = "10:30 昼食の調理・配膳準備を行う。..."
    - ステップ4: content = "10:50 食器洗い・台所の清掃を行う。..."
  - 時刻はパターンの開始時刻から順番に振り分け、終了時刻を超えないようにする
  - 各ケアの所要時間を考慮して自然な時間配分にする
- ★最重要：各ブロックのservice_typeに合ったステップのみ記載すること
  - service_type="身体介護"のブロック → 身体介護のケアのみ（体調確認・排泄・入浴・食事介助・移動・更衣・整容等）。家事は含めない
  - service_type="家事援助"のブロック → 家事援助のケアのみ（掃除・洗濯・調理・買い物・整理整頓・ゴミ捨て等）。身体は含めない
  - service_type="重度訪問介護"のブロック → 身体介護と家事援助が一体的に提供されるため混在OK
- ★重要：各ステップに「category」フィールドを必ず付ける。値は「身体介護」または「家事援助」のいずれか。
  - 身体に関するケア → category: "身体介護"
  - 家事に関するケア → category: "家事援助"
- 各ステップの「content」は時刻の後に具体的な介助方法・手順を記述する（「〜の介助」だけでなく「〜の状態を確認し、〜を介助する」等）
- 各ステップの「note」はその利用者固有の留意点を記述する（一般論ではなく個別性のある内容）

═══════════════════════════════════════════════════
■ 生成手順（必ずこの順番で思考すること）
═══════════════════════════════════════════════════
1. 【シフト・実績情報】の末尾にある【時間帯パターン判定結果】を確認する。ここに記載されたパターン数・サービス種別が正解
2. アセスメントを網羅的に読み取り、介助・援助が必要な全項目をリストアップする（ADL：移動・排泄・食事・入浴・更衣・整容、IADL：調理・洗濯・掃除・買い物・服薬管理等）
3. リストアップした全項目を、サービス種別（身体介護/家事援助）に従って該当するブロックに振り分ける
4. パターン数と同数のサービス内容ブロック（service1〜）を生成する。各ブロックのservice_typeはパターンのサービス種別と一致させる
5. 生成後、以下を自己検証する：
   - ブロック数が【時間帯パターン判定結果】のパターン数と一致しているか
   - 各ブロックのservice_typeがパターンのサービス種別と一致しているか
   - 身体介護ブロックに家事の内容が混ざっていないか、家事援助ブロックに身体の内容が混ざっていないか
...
【出力ルール】
1. ブロック数は【時間帯パターン判定結果】のパターン数と厳密に一致させる。パターンが1つならservice1のみ（service2〜4はnull）、2つならservice1とservice2のみ（service3・4はnull）。
2. 各ブロックのservice_typeは【時間帯パターン判定結果】のサービス種別と一致させること。
3. ★身体介護ブロックには身体系ケアのみ、家事援助ブロックには家事系ケアのみ記載。混ぜない。
4. 使用するブロック（service1〜）のstepsは5〜8項目とする。
5. 各サービス枠のstepsは、その訪問で実施するケアの流れ（手順）を時系列順に並べる。
6. visit_labelは「月〜金 午前の身体介護」「火・木 夕方の家事援助」等、曜日と時間帯とサービス種別を含める。
7. 重度訪問介護の場合、service_typeは「重度訪問介護」とし、身体・家事の混在を許可する。
8. contentは具体的な手順を40文字以上で記述する。「〜の介助」だけの短い記述は不可。
9. noteはその利用者の個別状況に基づく留意点を40文字以上で記述する。
10. 不要なサービス枠は必ずnullにする。パターン数以上のブロックを生成しない。`;
...
- 使用するサービスブロック（service1〜）は必ず5件以上のstepsを持つこと。ブロック数は【時間帯パターン判定結果】のパターン数と一致させ、各ブロックのservice_typeもパターンのサービス種別と一致させる。
...
## サービス枠の分け方（最重要）
- 【シフト・実績情報】の末尾に【時間帯パターン判定結果】が記載されている。そのパターン数・サービス種別がブロック構成の正解である
- ブロック数はパターン数と厳密に一致させ、各ブロックのservice_typeもパターンのサービス種別と一致させること
- ★最重要：身体介護ブロックには身体系ケアのみ、家事援助ブロックには家事系ケアのみを記載。絶対に混ぜないこと
- 重度訪問介護は例外で、身体介護と家事援助を1ブロック内に混在させてよい
- 同じ種別・同じ時間帯パターンなのに曜日ごとにブロックを分けない

## 文章品質の基準
- 「content」（援助内容）は先頭に「HH:MM 」形式の時刻を必ず付け、40〜60文字で具体的な手順・方法を記述
  - 例: "12:00 入室し、体調確認・バイタルチェックを行う。顔色・表情を観察し変化がないか確認する"
...
function buildBillingSummary(records: BillingRecord[]): string {
  ...
  // サービス種別×時間帯パターン集計: 「サービス種別＋開始時刻〜終了時刻」のユニークパターンを抽出
  // 同じ時間帯でもサービス種別（身体介護/家事援助等）が異なれば別パターンとする
  const serviceTimePatterns = new Set<string>();
  for (const r of records) {
    if (!r.startTime || !r.endTime) continue;
    const label = serviceCodeToLabel(r.serviceCode) || '不明';
    serviceTimePatterns.add(`${label} ${r.startTime}〜${r.endTime}`);
  }
  const patternList = [...serviceTimePatterns];
  ...
  lines.push(`→ 各ブロックのservice_typeは上記パターンのサービス種別と一致させること`);
  lines.push(`→ 各ブロックのstepsには、そのサービス種別に該当するケア内容のみを記載すること（身体介護ブロックに家事の内容を混ぜない、家事援助ブロックに身体の内容を混ぜない）`);
...
変更してよいのは短期目標の文言と期間のみです。`;
  ...
  if (ctx.planCreationDate) {
    ...
    planDate = new Date(year, month - 1, 1);
  ...
  // サービス内容チェックボックス（実際のサービス内容のcategory + service_typeから判定）
  // 全サービスブロックのstepsを集約してチェック
  const allPlanServices = [plan.service1, plan.service2, plan.service3, plan.service4].filter(Boolean) as ServiceBlock[];
  const hasBodyCategory = allPlanServices.some(s => s.steps.some(st => st.category === '身体介護'));
  const hasHouseCategory = allPlanServices.some(s => s.steps.some(st => st.category === '家事援助'));
  const allServiceTypes = allPlanServices.map(s => s.service_type || '');
  const hasHeavy = allServiceTypes.some(st => st.includes('重度訪問') || st.includes('重度'));
  const hasVisitBody = allServiceTypes.some(st => st.includes('通院') && (st.includes('伴う') || (!st.includes('伴う') && !st.includes('伴わない'))));
  const hasVisitNoBody = allServiceTypes.some(st => st.includes('通院') && st.includes('伴わない'));
  const hasRide = allServiceTypes.some(st => st.includes('乗降'));
  const hasAccompany = allServiceTypes.some(st => st.includes('同行'));
  const hasBehavior = allServiceTypes.some(st => st.includes('行動'));

  // 契約支給量から時間数を取得（チェックの有無は実サービスから判定）
  ...
  ws0.getCell('D16').value = checkboxText('身体介護', { checked: hasBodyCategory, hours: bodyHours });
  ws0.getCell('D16').alignment = checkboxAlignment;
  ws0.getCell('G16').value = checkboxText('家事援助', { checked: hasHouseCategory, hours: houseHours });
  ...
  for (let blockIdx = 0; blockIdx < serviceBlocks.length; blockIdx++) {
    ...
    // チェックボックス行: ステップのcategoryとservice_typeから正確に判定
    const blockFlags: CheckFlags = {
      body: false, house: false, heavy: false,
      visitBody: false, visitNoBody: false,
      ride: false, behavior: false, accompany: false,
    };
    // ステップのcategoryから身体介護・家事援助を判定
    if (bodySteps.length > 0) blockFlags.body = true;
    if (houseSteps.length > 0) blockFlags.house = true;
    // service_typeから重度訪問・通院・同行・行動を判定
    if (service?.service_type) {
      const stFlags = serviceTypeToCheckFlags(service.service_type);
      if (stFlags.heavy) blockFlags.heavy = true;
      if (stFlags.visitBody) blockFlags.visitBody = true;
      if (stFlags.visitNoBody) blockFlags.visitNoBody = true;
      if (stFlags.ride) blockFlags.ride = true;
      if (stFlags.accompany) blockFlags.accompany = true;
      if (stFlags.behavior) blockFlags.behavior = true;
    }
