...
export async function generate(ctx: GeneratorContext): Promise<void> {
  const { careClients, shifts, billingRecords, year, month, officeInfo, hiddenDiv } = ctx;
...
- 利用サービス種別: ${serviceTypes.join(', ') || '不明'}
- 月間サービス回数: 約${totalVisits}回

【実績データ（${year}年${month}月）】
${billingDetails || shiftDetails || 'データなし'}

${assessmentFileUrls.length > 0 ? '【添付アセスメント資料】\n上記に添付した画像/PDFファイルはこの利用者のアセスメント記録です。内容を読み取って計画に反映してください。' : ''}
...
以下の項目をJSON形式で出力してください:
{
  ...
  "family_wish": "家族の意向・希望"
    ...
    const systemInstruction = '訪問介護事業所のサービス提供責任者として、居宅介護計画書を作成してください。実績データとアセスメント資料を元に、具体的で実践的な計画を立案してください。必ず有効なJSON形式で出力してください。';
