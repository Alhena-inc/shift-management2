...
■ 身体介護の枠に記載する項目：
  ...
  バイタルチェック・就寝準備・食事介助・口腔ケア・体調確認 など
...
- 各ステップの「content」は時刻の後に具体的な介助方法・手順を記述する（40〜60文字）
- 各ステップの「note」はその利用者固有の留意点を記述する（40〜60文字）
...
- ★最重要：各枠にはそのサービス種別の援助項目のみ記載。絶対に混ぜないこと
  ...
  - 身体介護枠：服薬確認・更衣介助・整容介助・排泄介助・入浴介助・移乗介助・バイタルチェック・食事介助・口腔ケア
...
変更してよいのは短期目標の文言と期間のみです。`;
  ...
  const NIGHT_REALISTIC_BODY = /体位変換|排泄|服薬|水分補給|見守り|安全確認|体調確認|就寝|睡眠|夜間対応/;
  for (let i = 1; i <= 4; i++) {
    const service = plan[`service${i}` as keyof CarePlan] as ServiceBlock | null;
    if (!service || service.steps.length === 0) continue;
    ...
    if (isNightDeep) {
      ...
      service.steps = service.steps.filter(step => {
        const text = `${step.item} ${step.content}`;
        if (NIGHT_UNREALISTIC.test(text)) {
