...
- ★重要：各ステップの「content」の先頭に、そのケアを実施する具体的な時刻を記載すること
  - 例：パターンが12:00〜13:00の場合
    - ステップ1: content = "12:00 入室し、体調確認・バイタルチェックを行う。..."
    - ステップ2: content = "12:10 排泄介助。トイレへの移動を見守り、..."
    - ステップ3: content = "12:20 食事の準備・配膳を行い、..."
    - ステップ4: content = "12:40 食事介助を行い、..."
    - ステップ5: content = "12:50 口腔ケア・後片付けを行い、..."
...
- ★重要：各ステップに「category」フィールドを必ず付ける。値は「身体介護」または「家事援助」のいずれか。
  - 身体に関するケア（体調確認、排泄介助、入浴介助、移動介助、更衣、整容等）→ category: "身体介護"
  - 家事に関するケア（掃除、洗濯、調理、買い物、整理整頓、ゴミ捨て等）→ category: "家事援助"
  - 重度訪問介護でも各ステップにcategoryを付けること
- ★重要：stepsは身体介護のステップを先にまとめ、家事援助のステップを後にまとめて記載すること（身体介護→家事援助の順）
...
- 備考欄に他サービス（デイサービス・支援学校・就労支援等）の利用時間帯・曜日を記載する場合、計画予定表（シフト・実績情報）と矛盾しないか必ず検証すること
...
変更してよいのは短期目標の文言と期間のみです。`;
  ...
  for (let blockIdx = 0; blockIdx < serviceBlocks.length; blockIdx++) {
    ...
    // チェックボックス行: ステップのcategoryとservice_typeから判定
    let blockFlags: CheckFlags;
    if (service?.service_type) {
      blockFlags = serviceTypeToCheckFlags(service.service_type);
      // ステップのcategoryからも補完（重度訪問介護で身体+家事混在の場合）
      if (bodySteps.length > 0) blockFlags.body = true;
      if (houseSteps.length > 0) blockFlags.house = true;
    } else {
      blockFlags = globalCheckFlags;
    }

    const chk = block.chkStartRow;
    const chkCells = [
      [`B${chk}`, checkboxTextBack('身体介護', blockFlags.body)],
      [`F${chk}`, checkboxTextBack('家事援助', blockFlags.house)],
      [`H${chk}`, checkboxTextBack('重度訪問介護', blockFlags.heavy)],
      [`B${chk + 1}`, checkboxTextBack('通院等介助(身体介護を伴う)', blockFlags.visitBody)],
      [`F${chk + 1}`, checkboxTextBack('通院等介助(身体介護を伴わない)', blockFlags.visitNoBody)],
      [`B${chk + 2}`, checkboxTextBack('通院等乗降介助', blockFlags.ride)],
      [`F${chk + 2}`, checkboxTextBack('行動援護', blockFlags.behavior)],
      [`H${chk + 2}`, checkboxTextBack('同行援護', blockFlags.accompany)],
    ] as const;
    for (const [ref, val] of chkCells) {
