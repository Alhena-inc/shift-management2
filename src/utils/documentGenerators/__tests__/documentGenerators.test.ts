...
describe('F60/G60/H60/J60 身体介護サマリー', () => {
  it('身体介護セルは固定サマリー「体調確認・服薬確認・更衣介助・整容支援・安全確認・相談援助」', () => {
    const FIXED = '体調確認・服薬確認・更衣介助・整容支援・安全確認・相談援助';
    expect(FIXED).toContain('体調確認');
    expect(FIXED).toContain('服薬確認');
    expect(FIXED).toContain('更衣介助');
...
describe('B89-J93 身体介護ステップ正規化', () => {
  it('排泄介助が含まれず、更衣介助・整容介助・安全確認・相談援助が含まれること', () => {
    const steps = [
      { item: '訪問時確認' }, { item: '服薬確認' }, { item: '更衣介助' },
      { item: '整容介助' }, { item: '安全確認・相談援助' }, { item: '就寝準備・終了確認' },
    ];
    const items = steps.map(s => s.item);
    expect(items).not.toContain('排泄介助');
    expect(items).toContain('更衣介助');
...
describe('手順書 1930 身体介護ステップ', () => {
  it('排泄介助が含まれず、更衣介助・整容介助・安全確認・相談援助が含まれること', () => {
    const items = ['訪問時確認', '服薬確認', '更衣介助', '整容介助', '安全確認・相談援助', '就寝準備・退室前安全確認'];
    expect(items).not.toContain('排泄介助');
...
describe('モニタリング M12 文言', () => {
  it('M12に排泄介助がなく更衣介助・整容支援・安全確認・相談援助があること', () => {
...
describe('モニタリング C21 手順書確認結果', () => {
  it('C21に排泄介助がなく更衣介助・整容支援・安全確認・傾聴があること', () => {
    const text = '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、抗酒剤の服薬状況確認、更衣介助、必要時の整容支援、室内安全確認、相談・傾聴、就寝準備および退室前安全確認の手順について、現在の手順で支援が適切に実施されている。';
    expect(text).not.toContain('排泄');
    expect(text).toContain('更衣介助');
