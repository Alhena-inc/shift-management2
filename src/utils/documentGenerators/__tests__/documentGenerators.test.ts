...
describe('F60/G60/H60/J60 身体介護サマリー', () => {
  it('身体介護セルは固定サマリー「体調確認・服薬確認・安全確認・相談援助」', () => {
    const FIXED = '体調確認・服薬確認・安全確認・相談援助';
...
describe('B89-J93 身体介護ステップ正規化', () => {
  it('排泄介助・更衣・整容が含まれず、安全確認・相談・傾聴が含まれること', () => {
    const steps = [
      { item: '訪問時確認' }, { item: '服薬確認' }, { item: '安全確認' },
      { item: '相談・傾聴' }, { item: '就寝準備・終了確認' },
    ];
    const items = steps.map(s => s.item);
    expect(items).not.toContain('排泄介助');
    expect(items).not.toContain('更衣支援');
    expect(items).not.toContain('洗面・整容支援');
    expect(items).toContain('安全確認');
    expect(items).toContain('相談・傾聴');
  });
});

describe('手順書 1930 身体介護ステップ', () => {
  it('排泄介助が含まれず、食事見守り・水分補給・整容・移動見守りが含まれること', () => {
    const items = ['訪問時確認', '服薬確認', '食事見守り', '水分補給促し', '食後整容支援', '必要時移動見守り'];
    expect(items).not.toContain('排泄介助');
    expect(items).toContain('食事見守り');
    expect(items).toContain('水分補給促し');
    expect(items).toContain('必要時移動見守り');
  });
});

describe('モニタリング M12 文言', () => {
  it('M12に排泄介助がなく食事見守り・移動見守り・整容支援があること', () => {
    const fallback = '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、身体介護時の服薬確認・食事見守り・必要時の移動見守りおよび整容支援が適切に実施され、在宅生活の安定が図られている。';
    expect(fallback).not.toContain('排泄');
    expect(fallback).toContain('食事見守り');
    expect(fallback).toContain('移動見守り');
    expect(fallback).toContain('整容支援');
  });
});

describe('モニタリング C21 手順書確認結果', () => {
  it('C21に排泄介助がなく食事見守り・整容支援・移動見守りがあること', () => {
    const text = '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、食事見守り、水分補給の促し、必要時の整容支援および移動見守りの手順について、現在の手順で支援が適切に実施されている。';
    expect(text).not.toContain('排泄');
    expect(text).toContain('食事見守り');
