...
describe('F60/G60/H60/J60 身体介護サマリー', () => {
  it('身体介護セルは固定サマリー「体調確認・服薬確認・食事見守り」', () => {
    const FIXED = '体調確認・服薬確認・食事見守り';
    expect(FIXED).toContain('体調確認');
    expect(FIXED).toContain('服薬確認');
    expect(FIXED).toContain('食事見守り');
...
describe('B89-J93 身体介護ステップ正規化', () => {
  it('排泄介助が含まれず、食事見守り・移動見守り・整容支援が含まれること', () => {
    const steps = [
      { item: '訪問時確認' }, { item: '服薬確認' }, { item: '食事見守り' },
      { item: '必要時移動見守り' }, { item: '必要時整容支援・終了確認' },
    ];
