...
// ==================== 松尾光雅ケース: current journals 統一テスト（v4） ====================

describe('F60/G60/H60/J60 身体介護サマリー', () => {
  it('身体介護セルは固定サマリー「体調確認・服薬確認・排泄介助・食事見守り」', () => {
    // 実コードは身体介護の場合、ステップ内容に関係なく固定サマリーを返す
    const FIXED_BODY_SUMMARY = '体調確認・服薬確認・排泄介助・食事見守り';
    expect(FIXED_BODY_SUMMARY).toContain('体調確認');
    expect(FIXED_BODY_SUMMARY).toContain('服薬確認');
    expect(FIXED_BODY_SUMMARY).toContain('排泄介助');
    expect(FIXED_BODY_SUMMARY).toContain('食事見守り');
    expect(FIXED_BODY_SUMMARY).not.toContain('入浴');
    expect(FIXED_BODY_SUMMARY).not.toContain('健康チェック');
  });
});
...
describe('K21身体介護サマリー固定ロジック', () => {
  ...
  function normalizeK21(remarks: string): string {
    ...
    if (bodySummaryMatch) {
      return remarks.replace(bodySummaryMatch[0], `身体介護（${BODY_SUMMARY}）`);
    } else if (/身体介護/.test(remarks)) {
      return remarks.replace(/身体介護(?![（(])/, `身体介護（${BODY_SUMMARY}）`);
    }
    return remarks;
  }

  it('「身体介護（服薬確認・見守り）」→ 固定サマリーに置換', () => {
    const result = normalizeK21('身体介護（服薬確認・見守り）');
    expect(result).toBe(`身体介護（${BODY_SUMMARY}）`);
  });

  it('「身体介護（服薬確認・健康チェック）」→ 固定サマリーに置換', () => {
    const result = normalizeK21('身体介護（服薬確認・健康チェック）');
    expect(result).toBe(`身体介護（${BODY_SUMMARY}）`);
  });

  it('裸の「身体介護。」→ 固定サマリーで具体化', () => {
    const result = normalizeK21('水曜身体介護。');
    expect(result).toContain(`身体介護（${BODY_SUMMARY}）`);
  });

  it('家事援助部分は変更しない', () => {
    const result = normalizeK21('家事援助（調理・掃除）、身体介護（服薬確認・声かけ見守り）');
...
describe('B89-J93 身体介護ステップ正規化', () => {
  it('正規化されたステップに訪問時確認・服薬確認・排泄介助・食事見守り・終了確認が含まれる', () => {
    const steps = [
      { item: '訪問時確認', content: '表情・体調・食欲の確認', note: '著変や食欲低下の有無を確認する' },
      { item: '服薬確認', content: '本日分の服薬確認と声かけ', note: '漢方薬への抵抗感に配慮し確実な服薬を確認する' },
      { item: '排泄介助', content: 'トイレ誘導・排泄後の清拭等を行う', note: '清潔保持と転倒予防に配慮する' },
      { item: '食事見守り', content: '食事量・水分摂取量の確認と見守り', note: 'むせ込みや摂取状況に注意する' },
      { item: '終了確認', content: '退室前の体調確認と安全確認', note: '不安や訴えを傾聴する' },
    ];
    expect(steps.map(s => s.item)).toEqual(['訪問時確認', '服薬確認', '排泄介助', '食事見守り', '終了確認']);
    expect(steps[1].note).toContain('漢方薬');
  });
});

describe('手順書 1930 身体介護ステップ', () => {
  it('デフォルト身体介護ステップに訪問時確認・服薬確認・排泄介助・食事見守りが含まれる', () => {
    const steps = [
      { item: '訪問時確認' },
      { item: '服薬確認' },
      { item: '排泄介助' },
    ...
    expect(steps.map(s => s.item)).toEqual(['訪問時確認', '服薬確認', '排泄介助', '食事見守り']);
  });
});

describe('モニタリング M12 文言', () => {
  it('M12フォールバック文言に「服薬確認・排泄介助・食事見守り支援」がある', () => {
    const fallback = '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、身体介護時の服薬確認・排泄介助・食事見守り支援の継続により、在宅生活の安定が図られている。';
    expect(fallback).toContain('排泄介助');
    expect(fallback).toContain('食事見守り');
    expect(fallback).toContain('服薬確認');
    expect(fallback).not.toContain('入浴');
  });
});

describe('モニタリング C21 手順書確認結果', () => {
  it('C21デフォルト文言に排泄介助および食事見守りが含まれること', () => {
    const defaultText = '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、排泄介助および食事見守りの手順について、現在の手順で支援が適切に実施されている。';
    expect(defaultText).toContain('排泄介助および食事見守り');
    expect(defaultText).toContain('服薬');
    expect(defaultText).not.toContain('入浴介助');
    expect(defaultText).toMatch(/。$/);
  });

  it('未完文ガード: 「服薬している」で終わる文は「。」が付かない', () => {
    const text = '手順書の内容を確認した結果、服薬している';
    expect(text.endsWith('。')).toBe(false);
  });
});

describe('日誌本文 meal wording 統一', () => {
  it('mealAssist variants から「食事介助」「食事の介助」が消えていること', () => {
    const mealVariants = [
      '食事量と水分摂取量を確認し、食事の見守りと声かけを行った',
      '食事の配膳を行い、摂取状況を確認しながら見守りと声かけを行った',
      '食事量と水分摂取量を確認し、摂取状況の見守りと声かけを行った',
    ];
    for (const v of mealVariants) {
      expect(v).not.toContain('食事介助');
      expect(v).not.toContain('食事の介助');
      expect(v).not.toContain('配膳・介助');
    }
  });

  it('最終クリーンアップが「食事介助」を除去すること', () => {
    let text = '食事の介助を行い、摂取状況を確認した。食事介助を行った。配膳・介助を行った。';
    text = text
    ...
    expect(text).not.toContain('食事の介助');
    expect(text).not.toContain('配膳・介助');
    expect(text).toContain('見守り');
  });
});
