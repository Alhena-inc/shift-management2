...
// ==================== 松尾光雅ケース: current operation 統一テスト ====================

describe('F60/G60/H60/J60 身体介護サマリー正規化', () => {
  // getRepresentativeItems V3: 実コード準拠（入浴→入浴支援、更衣/整容→身辺介助、健康チェック除去）
  function getRepresentativeItemsV3(
    serviceBlocks: Array<{ service_type: string; steps: Array<{ item: string }> }>,
    serviceType: string,
  ): string {
    if (!serviceBlocks || serviceBlocks.length === 0) return '';
    ...
    if (!block || block.steps.length === 0) return '';
    const meaningful = block.steps.filter(s => !/到着|挨拶|退室|訪問開始|バイタル|体温測定/.test(s.item));
    const items = meaningful.length > 0 ? meaningful : block.steps;
  ...
  it('松尾光雅ケース: 「服薬確認・入浴介助・更衣介助・整容介助」→「服薬確認・入浴支援・身辺介助」', () => {
    const blocks = [
      { service_type: '身体介護', steps: [
        { item: '服薬確認' }, { item: '入浴介助' }, { item: '更衣介助' }, { item: '整容介助' }, { item: '就寝準備' },
      ] },
    ];
    const result = getRepresentativeItemsV3(blocks, '身体介護');
    expect(result).toBe('服薬確認・入浴支援・身辺介助');
  });

  it('「入浴準備」→「入浴支援」に正規化されること', () => {
    const blocks = [
      { service_type: '身体介護', steps: [
        { item: '服薬確認' }, { item: '入浴準備' }, { item: '更衣・整容介助' },
      ] },
    ];
    const result = getRepresentativeItemsV3(blocks, '身体介護');
    expect(result).toContain('入浴支援');
    expect(result).not.toContain('入浴準備');
  });

  it('「健康チェック」「体調確認」はサマリーに入らない', () => {
    const blocks = [
      { service_type: '身体介護', steps: [
        { item: '健康チェック' }, { item: '服薬確認' }, { item: '入浴介助' },
      ] },
    ];
    const result = getRepresentativeItemsV3(blocks, '身体介護');
    expect(result).not.toContain('健康チェック');
    expect(result).not.toContain('体調確認');
    expect(result).toContain('服薬確認');
    expect(result).toContain('入浴支援');
  });

  it('家事援助の「調理・掃除」は維持されること', () => {
    const blocks = [
      { service_type: '家事援助', steps: [
        { item: '調理支援' }, { item: '掃除' }, { item: '洗濯' },
      ] },
    ];
    const result = getRepresentativeItemsV3(blocks, '家事援助');
    expect(result).toBe('調理支援・掃除・洗濯');
  });
});

describe('K21身体介護サマリー整合ロジック（v3）', () => {
  // K21 body summary post-processing: 健康チェック除去、見守り→必要時見守り、入浴補充
  function normalizeK21BodySummaryV3(remarks: string, allStepText: string): string {
    const bodySummaryMatch = remarks.match(/身体介護[（(]([^）)]+)[）)]/);
    const bodyHasBath = /入浴|シャワー|清拭/.test(allStepText);
    const bodyHasMed = /服薬|薬.*確認/.test(allStepText);

    if (bodySummaryMatch) {
      let items = bodySummaryMatch[1].split(/[・、,]/).map(item => {
        if (/体温測定|バイタル|健康チェック/.test(item)) return null;
        if (/^入浴準備$/.test(item.trim())) return '入浴支援';
        if (/^見守り$/.test(item.trim())) return '必要時見守り';
        if (/食事見守/.test(item)) return null;
        return item;
      }).filter((i): i is string => i !== null);
      if (bodyHasBath && !items.some(i => /入浴/.test(i))) items.push('入浴支援');
      if (bodyHasMed && !items.some(i => /服薬/.test(i))) items = ['服薬確認', ...items];
      if (!items.some(i => /見守/.test(i)) && /更衣|整容/.test(allStepText)) items.push('必要時見守り');
      const unique = [...new Set(items)];
      return remarks.replace(bodySummaryMatch[0], `身体介護（${unique.join('・')}）`);
    } else if (/身体介護/.test(remarks) && (bodyHasBath || bodyHasMed)) {
      const parts: string[] = [];
      if (bodyHasMed) parts.push('服薬確認');
      if (bodyHasBath) parts.push('入浴支援');
      if (/更衣|整容/.test(allStepText)) parts.push('必要時見守り');
      if (parts.length > 0) {
        return remarks.replace(/身体介護(?![（(])/, `身体介護（${parts.join('・')}）`);
      }
    }
    return remarks;
  }

  it('松尾光雅ケース: 「身体介護（服薬確認・見守り）」→「身体介護（服薬確認・入浴支援・必要時見守り）」', () => {
    const steps = '服薬確認 処方薬の確認 入浴介助 シャワー浴 更衣介助 着替え';
    const result = normalizeK21BodySummaryV3('身体介護（服薬確認・見守り）', steps);
    expect(result).toContain('服薬確認');
    expect(result).toContain('入浴支援');
    expect(result).toContain('必要時見守り');
    expect(result).not.toContain('・見守り）');
  });

  it('裸の「身体介護」を具体化', () => {
    const steps = '服薬確認 薬 入浴介助 シャワー浴 更衣介助';
    const result = normalizeK21BodySummaryV3('水曜身体介護。', steps);
    expect(result).toContain('身体介護（服薬確認・入浴支援・必要時見守り）');
  });

  it('裸の「家事援助」を具体化', () => {
    const steps = '調理 料理 清掃 掃除 服薬確認 薬 入浴介助 シャワー浴 更衣介助';
    const result = normalizeK21BodySummaryV3('水曜家事援助、木曜身体介護。', steps);
    // 身体介護側が具体化される（家事援助の具体化は別ロジックで処理）
    expect(result).toContain('身体介護（服薬確認・入浴支援・必要時見守り）');
  });

  it('「健康チェック」はK21から除去される', () => {
    const steps = '服薬確認 薬 入浴介助 シャワー';
    const result = normalizeK21BodySummaryV3('身体介護（服薬確認・健康チェック）', steps);
    expect(result).not.toContain('健康チェック');
    expect(result).toContain('服薬確認');
    expect(result).toContain('入浴支援');
  });

  it('「食事見守り」はK21から除去される', () => {
    const steps = '服薬確認 薬 入浴介助 シャワー';
    const result = normalizeK21BodySummaryV3('身体介護（服薬確認・食事見守り）', steps);
    expect(result).not.toContain('食事見守り');
    expect(result).toContain('入浴支援');
  });
});

describe('モニタリング M12 文言', () => {
  it('松尾光雅ケース: M12フォールバック文言に「入浴時の見守り・介助支援」がある', () => {
    const fallback = '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。手の震えに配慮した調理支援、意欲低下時の清潔保持支援、服薬状況の毎回確認、入浴時の見守り・介助支援の継続により、現状の生活が安定して維持できている。';
    expect(fallback).toContain('入浴時の見守り・介助支援');
    expect(fallback).toContain('調理支援');
    expect(fallback).toContain('服薬');
    expect(fallback).not.toContain('食事見守り支援');
  });
});

describe('モニタリング C21 手順書確認結果', () => {
  it('松尾光雅ケース: C21デフォルト文言に入浴介助・更衣介助・整容介助が含まれること', () => {
    const defaultText = '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、入浴介助・更衣介助・整容介助の手順について、現在の手順で支援が適切に実施されている。';
    expect(defaultText).toContain('調理支援');
    expect(defaultText).toContain('臨機応変');
    expect(defaultText).toContain('服薬');
    expect(defaultText).toContain('入浴介助・更衣介助・整容介助');
    // 未完文でないこと
    expect(defaultText).toMatch(/。$/);
  });

  it('未完文検出: 「服薬している」で終わる文は未完文と判定される', () => {
    const text = '手順書の内容を確認した結果、服薬している';
    const INCOMPLETE_END = /[はがをにでもへとのやかし、,]$/;
    const body = text.trimEnd();
    const isIncomplete = INCOMPLETE_END.test(body.replace(/。$/, ''));
    // 「る」は助動詞だが上記パターンには含まれない
    // しかし「。」で終わっていないので不完全と判定
