...
describe('K21/F60身体介護サマリー正規化', () => {
  // getRepresentativeItems のロジック再現（実コード準拠: 入浴準備→入浴支援、健康チェック→体調確認）
  function getRepresentativeItemsV2(
    serviceBlocks: Array<{ service_type: string; steps: Array<{ item: string }> }>,
    serviceType: string,
  ): string {
    if (!serviceBlocks || serviceBlocks.length === 0) return '';
    ...
    const items = meaningful.length > 0 ? meaningful : block.steps;
    return [...new Set(items.slice(0, 3).map(s => {
  ...
  it('「入浴準備」→「入浴支援」に正規化されること', () => {
    const blocks = [
      { service_type: '身体介護', steps: [
        { item: '体調確認' }, { item: '服薬確認' }, { item: '入浴準備' },
      ] },
    ];
    const result = getRepresentativeItemsV2(blocks, '身体介護');
    expect(result).toBe('体調確認・服薬確認・入浴支援');
    expect(result).not.toContain('入浴準備');
  });

  it('「健康チェック」→「体調確認」に正規化されること', () => {
    const blocks = [
      { service_type: '身体介護', steps: [
        { item: '健康チェック' }, { item: '服薬確認' }, { item: '入浴準備' },
      ] },
    ];
    const result = getRepresentativeItemsV2(blocks, '身体介護');
    expect(result).toBe('体調確認・服薬確認・入浴支援');
  ...
  it('家事援助の「調理・掃除」は維持されること', () => {
    ...
    const result = getRepresentativeItemsV2(blocks, '家事援助');
    expect(result).toBe('調理支援・掃除・洗濯');
  });
});

describe('K21身体介護サマリー整合ロジック', () => {
  // K21 body summary post-processing logic再現
  function normalizeK21BodySummary(remarks: string): string {
    const bodySummaryMatch = remarks.match(/身体介護[（(]([^）)]+)[）)]/);
    if (!bodySummaryMatch) return remarks;
    const items = bodySummaryMatch[1].split(/[・、,]/).map(item => {
      if (/体温測定|体温確認|バイタルチェック|バイタル測定|バイタル確認|健康チェック/.test(item)) return '体調確認';
  ...
  it('「身体介護（服薬確認・健康チェック）」→「身体介護（服薬確認・体調確認）」', () => {
    const result = normalizeK21BodySummary('身体介護（服薬確認・健康チェック）');
    expect(result).toBe('身体介護（服薬確認・体調確認）');
    expect(result).not.toContain('健康チェック');
  });

  it('「身体介護（入浴準備・服薬確認）」→「身体介護（入浴支援・服薬確認）」', () => {
    const result = normalizeK21BodySummary('身体介護（入浴準備・服薬確認）');
    expect(result).toBe('身体介護（入浴支援・服薬確認）');
    expect(result).not.toContain('入浴準備');
  });

  it('松尾光雅ケース: 2026年1月K21「身体介護（体調確認・服薬確認・入浴支援）」', () => {
    const input = '身体介護（体調確認・服薬確認・入浴準備）、家事援助（調理・掃除）';
    const result = normalizeK21BodySummary(input);
    expect(result).toContain('身体介護（体調確認・服薬確認・入浴支援）');
    expect(result).toContain('家事援助（調理・掃除）');
    expect(result).not.toContain('入浴準備');
  });
});

describe('モニタリング M12 文言正規化', () => {
  // TYPO_FIXES の正規化ロジック再現
  function applyMonitoringFixes(text: string): string {
  ...
  it('「食事見守り支援」→「入浴支援」に置換', () => {
    const input = '身体介護時の体調確認・服薬確認・食事見守り支援の継続';
    const result = applyMonitoringFixes(input);
    expect(result).toContain('入浴支援');
    expect(result).not.toContain('食事見守り支援');
  });

  it('「清掃を中心とした」→「調理・掃除を中心とした」に置換', () => {
    const input = '清掃を中心とした家事援助と、身体介護時の体調確認';
    const result = applyMonitoringFixes(input);
    expect(result).toContain('調理・掃除を中心とした家事援助');
    expect(result).not.toContain('清掃を中心とした家事援助');
  });

  it('松尾光雅ケース: M12フォールバック文言が正しいこと', () => {
    const fallback = '確認した結果、現行サービス内容で対応可能であり変更は不要と判断した。調理・掃除を中心とした家事援助と、身体介護時の体調確認・服薬確認・入浴支援の継続により、現状の生活が安定して維持できている。';
    expect(fallback).toContain('調理・掃除を中心とした家事援助');
    expect(fallback).toContain('入浴支援');
    expect(fallback).not.toContain('食事見守り支援');
    expect(fallback).not.toContain('清掃を中心とした家事援助');
  });
});

describe('モニタリング C21 手順書確認結果', () => {
  it('松尾光雅ケース: C21デフォルト文言に入浴準備・見守り・更衣介助が含まれること', () => {
    const defaultText = '手順書の内容を確認した結果、現行の手順書で適切に対応できており変更は不要と判断した。手の震えに配慮した調理支援の方法、掃除箇所をその日の状況に応じて臨機応変に対応する運用、服薬状況の毎回確認、入浴準備・見守り・更衣介助の手順について、現在の手順で支援が適切に実施されている。';
    expect(defaultText).toContain('調理支援');
    expect(defaultText).toContain('臨機応変');
    expect(defaultText).toContain('服薬');
    expect(defaultText).toContain('入浴準備・見守り・更衣介助');
  });
});
