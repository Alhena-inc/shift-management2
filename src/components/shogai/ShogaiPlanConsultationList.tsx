...
import type { ShogaiPlanConsultation } from '../../types';
import { saveShogaiPlanConsultation, deleteShogaiPlanConsultation } from '../../services/dataService';

const OFFICE_OPTIONS = ['訪問介護事業所のあ'];
const SPECIALIST_OPTIONS = ['訪問介護事業所のあ'];

interface Props {
  careClientId: string;
  items: ShogaiPlanConsultation[];
  onUpdate: (items: ShogaiPlanConsultation[]) => void;
  onBack: () => void;
}
...
const ShogaiPlanConsultationList: React.FC<Props> = ({ careClientId, items, onUpdate, onBack }) => {
  const [view, setView] = useState<View>('list');
  const [editIndex, setEditIndex] = useState<number | null>(null);
  const [saving, setSaving] = useState(false);
  const [formData, setFormData] = useState<ShogaiPlanConsultation>({
    id: '', careClientId, consultationOffice: '', consultationSpecialist: '', validFrom: '', validUntil: '', sortOrder: 0,
  });

  const handleAdd = () => {
    setEditIndex(null);
    setFormData({ id: '', careClientId, consultationOffice: '', consultationSpecialist: '', validFrom: '', validUntil: '', sortOrder: items.length });
    setView('form');
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const saved = await saveShogaiPlanConsultation(formData);
      if (editIndex !== null) {
        const updated = [...items];
  ...
  const handleDelete = async (index: number) => {
    if (!confirm('この計画相談支援を削除しますか？')) return;
    try {
      await deleteShogaiPlanConsultation(items[index].id);
      ...
      <div>
        ...
        <div className="space-y-5">
          <div className="flex items-center gap-4">
            ...
            <select value={formData.consultationOffice} onChange={(e) => setFormData({ ...formData, consultationOffice: e.target.value })} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white text-sm">
              <option value="">選択してください</option>
              {OFFICE_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-4">
            <label className="text-sm font-medium text-gray-700 w-28 text-right shrink-0">相談支援専門員</label>
            <select value={formData.consultationSpecialist} onChange={(e) => setFormData({ ...formData, consultationSpecialist: e.target.value })} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white text-sm">
              <option value="">選択してください</option>
              {SPECIALIST_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
            </select>
          </div>
    ...
    <div>
      ...
      <div className="border border-gray-300 rounded-lg overflow-hidden">
        <div className="flex items-center gap-3 bg-gray-100 px-4 py-3">
          <span className="font-bold text-gray-800">計画相談支援</span>
        </div>
        <div className="divide-y divide-gray-200">
          ...
          {items.map((item, index) => (
            <div key={item.id} className="flex items-center px-4 py-3 hover:bg-gray-50 cursor-pointer gap-6" onClick={() => handleEdit(index)}>
              <span className="text-sm text-gray-800">{item.consultationOffice}</span>
              <span className="text-sm text-gray-800">{item.consultationSpecialist}</span>
...
export default ShogaiPlanConsultationList;
