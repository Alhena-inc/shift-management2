...
import type { ShogaiUsedService } from '../../types';
import { saveShogaiUsedService, deleteShogaiUsedService } from '../../services/dataService';
...
const ShogaiUsedServiceList: React.FC<Props> = ({ careClientId, items, onUpdate, onBack, source = 'shogai' }) => {
  ...
  const [formData, setFormData] = useState<ShogaiUsedService>({
    id: '', careClientId, serviceType: '', serviceStartDate: '', serviceEndDate: '', sortOrder: 0,
  });

  const handleAdd = () => {
    setEditIndex(null);
    setFormData({ id: '', careClientId, serviceType: '', serviceStartDate: '', serviceEndDate: '', sortOrder: items.length });
    setView('form');
  };
  ...
  const handleDelete = async (index: number) => {
    if (!confirm('この利用サービスを削除しますか？')) return;
    try {
      await deleteShogaiUsedService(items[index].id);
  ...
  const handlePeriodButton = (months: number) => {
    if (!formData.serviceStartDate) { alert('開始日を先に入力してください'); return; }
    setFormData({ ...formData, serviceEndDate: addPeriod(formData.serviceStartDate, months) });
  };
      ...
      <div>
        ...
        <div className="space-y-5">
          <div className="flex items-center gap-4">
            <label className="text-sm font-medium text-gray-700 w-28 text-right shrink-0">サービス種類</label>
            <select value={formData.serviceType} onChange={(e) => setFormData({ ...formData, serviceType: e.target.value })} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm">
          ...
          <div className="flex items-start gap-4">
            <label className="text-sm font-medium text-gray-700 w-28 text-right shrink-0 pt-2">サービス期間</label>
            <div className="flex items-center gap-2 flex-wrap">
              <input type="date" value={formData.serviceStartDate} onChange={(e) => setFormData({ ...formData, serviceStartDate: e.target.value })} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm" />
              <span className="text-gray-500">〜</span>
              <input type="date" value={formData.serviceEndDate} onChange={(e) => setFormData({ ...formData, serviceEndDate: e.target.value })} className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent text-sm" />
    ...
    <div>
      ...
      <div className="border border-gray-300 rounded-lg overflow-hidden">
        <div className="flex items-center gap-3 bg-gray-100 px-4 py-3">
          <span className="font-bold text-gray-800">利用サービス</span>
        </div>
        <div className="divide-y divide-gray-200">
          ...
          {items.map((item, index) => (
            <div key={item.id} className="flex items-center px-4 py-3 hover:bg-gray-50 cursor-pointer gap-6" onClick={() => handleEdit(index)}>
              <span className="text-sm text-gray-800 font-medium">{item.serviceType}</span>
              <span className="text-sm text-gray-600">
                {item.serviceStartDate && item.serviceEndDate ? `${toWareki(item.serviceStartDate)}〜${toWareki(item.serviceEndDate)}` : item.serviceStartDate ? `${toWareki(item.serviceStartDate)}〜` : ''}
              </span>
...
export default ShogaiUsedServiceList;
