...
import type { ShogaiBurdenLimitOffice } from '../../types';
import { saveShogaiBurdenLimitOffice, deleteShogaiBurdenLimitOffice } from '../../services/dataService';
...
const ShogaiBurdenLimitOfficeList: React.FC<Props> = ({ careClientId, items, onUpdate, onBack }) => {
  ...
  const [formData, setFormData] = useState<ShogaiBurdenLimitOffice>({
    id: '', careClientId, officeName: '', validFrom: '', validUntil: '', sortOrder: 0,
  });

  const handleAdd = () => {
    setEditIndex(null);
    setFormData({ id: '', careClientId, officeName: '', validFrom: '', validUntil: '', sortOrder: items.length });
    setView('form');
  };
  ...
  const handleDelete = async (index: number) => {
    if (!confirm('この利用者負担上限額管理事業所を削除しますか？')) return;
    try {
      await deleteShogaiBurdenLimitOffice(items[index].id);
    ...
    <div>
      ...
      <div className="border border-gray-300 rounded-lg overflow-hidden">
        <div className="flex items-center gap-3 bg-gray-100 px-4 py-3">
          <span className="font-bold text-gray-800">利用者負担上限額管理事業所</span>
        </div>
...
export default ShogaiBurdenLimitOfficeList;
