  ...
  try {
    ...
    const dataToSave = helpers.map(helper => {
      ...
      const saveData: any = {
        ...
        // 給与履歴（期間ごとの給与条件）
        salary_history: helper.salaryHistory || [],
    ...
    for (const helperData of dataToSave) {
      ...
      // insurance_history / salary_history カラムが未作成の場合は除外して再試行
      if (error && error.message && /(insurance_history|salary_history)/.test(error.message)) {
        console.warn('insurance_history/salary_history カラムが未作成のため除外して再試行');
        const { insurance_history: _ih, salary_history: _sh, ...withoutHistory } = helperData as any;
...
function helperRowToHelper(row: any, applyDefaults: boolean = true): Helper {
  ...
  const helper: any = {
    ...
    salaryHistory: row.salary_history ?? row.salaryHistory ?? (applyDefaults ? [] : undefined),
...
export const loadCareClients = async (): Promise<CareClient[]> => {
  try {
    ...
    return (data || []).map((row: any) => ({
      ...
      kantankaigoOrder: row.kantankaigo_order ?? undefined,
      name: row.name,
...
export const saveCareClient = async (client: CareClient): Promise<void> => {
  try {
    const saveData = {
      ...
      kantankaigo_order: client.kantankaigoOrder ?? null,
