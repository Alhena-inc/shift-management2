...
async function generatePayslipTemplate() {
  ...
  titleCell.value = `賃金明細 2025年 12月分(支払通知書)`;
  ...
  // 2. 基本情報（3行）
  // ==========================================
  const basicStartRow = currentRow;

  // 1行目：部署 | 介護事業 | 基本 | 2000 | 円 |
  ...
  basicRow1.getCell(4).value = 2000;
  ...
  basicRow2.getCell(5).value = 2000;
  ...
  basicRow3.getCell(2).value = '花山 様';
  ...
  basicRow3.getCell(5).value = 'アルバイト';
  ...
  attendanceLabel.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD6DCE5' } };
  ...
  // 空白セル
  attRow1.getCell(7).border = {
    top: { style: 'thin' },
    left: { style: 'thin' },
    right: { style: 'thin' },
    bottom: { style: 'thin' }
  };
  attRow1.getCell(8).border = {
    top: { style: 'thin' },
    left: { style: 'thin' },
    right: { style: 'thin' },
    bottom: { style: 'thin' }
  ...
  attRow2.getCell(7).border = {
    top: { style: 'thin' },
    left: { style: 'thin' },
    right: { style: 'thin' },
    bottom: { style: 'thin' }
  };
  attRow2.getCell(8).border = {
    top: { style: 'thin' },
    left: { style: 'thin' },
    right: { style: 'thin' },
    bottom: { style: 'thin' }
  ...
  const attendanceEndRow = currentRow - 1;
  ...
  paymentLabel.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD6DCE5' } };
  ...
  payRow4.getCell(6).border = {
    top: { style: 'thin' },
    left: { style: 'thin' },
    right: { style: 'thin' },
    bottom: { style: 'thin' }
  };
  payRow4.getCell(8).border = {
    top: { style: 'thin' },
    left: { style: 'thin' },
    right: { style: 'thin' },
    bottom: { style: 'thin' }
  ...
  const paymentEndRow = currentRow - 1;
  ...
  deductionLabel.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD6DCE5' } };
  ...
  const deductionEndRow = currentRow - 1;
  ...
  totalLabel.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD6DCE5' } };
  ...
  const totalEndRow = currentRow - 1;
  ...
  remarkLabel.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD6DCE5' } };
  ...
  // 勤怠項目の青太枠
  applyBlueBorder(worksheet, attendanceStartRow, attendanceEndRow, 1, 8, blueBorder);

  // 支給項目の青太枠
  applyBlueBorder(worksheet, paymentStartRow, paymentEndRow, 1, 8, blueBorder);

  // 控除項目の青太枠
  applyBlueBorder(worksheet, deductionStartRow, deductionEndRow, 1, 8, blueBorder);

  // 合計の青太枠
  applyBlueBorder(worksheet, totalStartRow, totalEndRow, 1, 8, blueBorder);
  ...
  // Excelファイルを保存
  await workbook.xlsx.writeFile('/Users/koike/Desktop/シフト/shift-table/賃金明細テンプレート.xlsx');
  console.log('✅ Excelファイルを生成しました: 賃金明細テンプレート.xlsx');
}

// 青太枠を適用するヘルパー関数
function applyBlueBorder(
  worksheet: ExcelJS.Worksheet,
  startRow: number,
  endRow: number,
  startCol: number,
  endCol: number,
  blueBorder: { style: 'medium'; color: { argb: string } }
) {
  // 上辺
  ...
  // 右辺
  for (let row = startRow; row <= endRow; row++) {
    const cell = worksheet.getRow(row).getCell(endCol);
    cell.border = {
      ...cell.border,
      right: blueBorder
    };
...
generatePayslipTemplate().catch(console.error);
